 ...
 * Main import handler
 */
async function handleImportFlow(flowData: FlowData, images: Record<string, number[]>) {
  try {
    sendProgress(5, '准备导入...');
 ...
 */
async function createFlowGraph(flowData: FlowData, images: Record<string, number[]>) {
  ...
  let containerParent: BaseNode & ChildrenMixin = figma.currentPage;
  ...
  for (let i = 0; i < keyframes.length; i++) {
    ...
    if (isFigJam) {
      ...
      (containerParent as FrameNode).appendChild(node);
    }
 ...
 */
async function createKeyframeNode(
  kf: KeyframeData, 
...
async function createConnection(
  ...
  if (figma.editorType === 'figjam') {
    try {
      const connector = (figma as any).createConnector();
